        ...
        axios.post("https://bcnscience-meetings.com/medicamente/chatbot/getBotResponse", data)
            .then(response => {
                ...
                setTimeout(function() {
                    ...
                    let botMessages = response.data.messages.map(msg => {
                        msg.isBotText = true;
                        return msg;
                    });
                    // get prev msgs and remove firstMessage flag if any
                    let messagesUnflagged = messages.map(m => {
                        m.firstMessage = false;
                        return m;
                    })
                    // add flag to scroll to that msg
                    botMessages[0].firstMessage = true;
                    // update state
                    let newMessages = messagesUnflagged.concat(botMessages);
                    setMessages(update(messages, {$merge: newMessages}));
